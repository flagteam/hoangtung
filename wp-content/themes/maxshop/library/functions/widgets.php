...
class alc_flickr_widget extends WP_Widget {
	...
	function alc_flickr_widget() {
		...
		// Widget settings.
		$widget_ops = array(
			'classname' => 'alc-flickr-widget',
			'description' => __('Display images from flickr', 'Maxshop')
		);
		...
		// Create the widget.
		$this->WP_Widget('alc-flickr-widget', __('Maxshop - Flickr images', 'Maxshop') , $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance) {
		...
		echo '<ul id="basicuse" class="flickr clearfix"></ul>'.$after_widget;
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('flickr_id'); ?>"><?php _e('Flickr ID:', 'Maxshop'); ?></label> 
        ...
        <p>
			<label for="<?php echo $this->get_field_id('flickr_nr'); ?>"><?php _e('Number of photos:', 'Maxshop'); ?></label> 
...
class Latest_Tweets extends WP_Widget {
	...
	function Latest_Tweets() {
		...
		$this->WP_Widget( 'latest-tweets-widget','Maxshop - Latest Tweets', $widget_ops, $control_ops );
	}
...
<li class="twitter-item"><i class="icon-twitter"></i>';
		                    ...
		                    if($update) {
		                      ...
		                      if ( ( abs( time() - $time) ) < 86400 )
		                        $h_time = sprintf( __('%s ago', 'Maxshop'), human_time_diff( $time ) );
		                      else
		                        $h_time = date(__('Y/m/d', 'Maxshop'), $time);
		                      echo sprintf( __('%s', 'twitter-for-wordpress', 'Maxshop'),' <span class="twitter-timestamp"><abbr title="' . date(__('Y/m/d H:i:s', 'Maxshop'), $time) . '">' . $h_time . '</abbr></span>' );
		                     }
		                    echo '</li>
';
		                    $i++;
...
';
				...
				{ 
					echo _e('Sorry , Twitter seems down or responds slowly.', 'Maxshop'); 
				}
            ?>
		<?php
		...
		<?php echo $after_title._e('You need to Setup Twitter API OAuth settings first', 'Maxshop');
		echo $after_widget;
	}
}
	...
	function form( $instance ) {
		$defaults = array( 'title' =>__('@Follow Me' , 'Maxshop') , 'no_of_tweets' => '5', 'twitter_username'=>'weblusive', 'consumer_key' => '', 'consumer_secret' => '', 'access_token' => '', 'access_token_secret' => '');
		$instance = wp_parse_args( (array) $instance, $defaults ); ?>
		
		<p>
			<label for="<?php echo $this->get_field_id( 'twitter_username' ); ?>"><?php _e('Username', 'Maxshop')?> : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'consumer_key' ); ?>"><?php _e('Consumer Key', 'Maxshop')?> : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'consumer_secret' ); ?>"><?php _e('Consumer Secret', 'Maxshop')?> : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'access_token' ); ?>"><?php _e('Access Token', 'Maxshop')?> : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'access_token_secret' ); ?>"><?php _e('Access Token Secret', 'Maxshop')?> : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'no_of_tweets' ); ?>"><?php _e('Number of Tweets to show', 'Maxshop')?> : </label>
...
/* ***************************************************** 
 * Plugin Name: Maxshop Blog Posts
...
class alc_blogposts_widget extends WP_Widget {
	...
	function alc_blogposts_widget() {
		...
		$widget_ops = array(
			...
			'description' => __('Display latest blog posts on footer', 'Maxshop')
		);

		// Create the widget.
		$this->WP_Widget('alc-blogposts-widget', __('Maxshop Footer Blog Posts', 'Maxshop') , $widget_ops);
	}
	...
	function form($instance) {
		...
		$instance = wp_parse_args((array)$instance, $defaults); ?>
		<p>
			<label for="<?php echo $this->get_field_id('post_title'); ?>"><?php _e('Title', 'Maxshop'); ?></label>
        ...
        <p>
			<label for="<?php echo $this->get_field_id('post_count'); ?>"><?php _e('Number of Posts to show', 'Maxshop'); ?></label> 
         ...
         <p>
			<label for="<?php echo $this->get_field_id('post_category'); ?>"><?php _e('Category (Leave Blank to show from all categories)', 'Maxshop'); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id('post_thumbs'); ?>"><?php _e('Show thumbnails', 'Maxshop'); ?></label>
...
class alc_totalposts_widget extends WP_Widget {
	...
	function alc_totalposts_widget() {
		...
		$widget_ops = array(
			...
			'description' => __('Retrieve and display popular/latest posts/latest comments.', 'Maxshop')
		);

		// Create the widget.
		$this->WP_Widget('alc-totalposts-widget', __('Maxshop Popular/Latest posts/Last comments', 'Maxshop') , $widget_ops);
	}
	...
	function widget($args, $instance) {
		...
		<ul class="nav nav-tabs">
			<li class="active"><a href="#pop" data-toggle="tab"><?php _e('Popular', 'Maxshop')?></a></li>
			<li><a href="#rec" data-toggle="tab"><?php _e('Recent', 'Maxshop')?></a></li>
			<li><a href="#com" data-toggle="tab"><?php _e('Comments', 'Maxshop')?></a></li>
		</ul>
		<div class="tab-content">
					...
					if ($myposts):
						foreach( $myposts as $post ) :	setup_postdata($post);  ?>                 
							<div class="media">
								...
								<div class="media-body">
									<h4 class="media-heading"><a href="<?php the_permalink()?>"><?php the_title()?></a></h4> 
									<p><?php echo limit_words( get_the_excerpt(), 9)?></p>
								</div>
							</div>
						<?php endforeach; 
					endif; ?>
			...
			<div class="tab-pane fade" id="com">
				...
				foreach ($comments as $comment) :?>
					<div class="media">
						<a href="<?php echo get_permalink($comment->ID).'#comment-'.$comment->comment_ID?>" title="<?php echo strip_tags($comment->comment_author).' '.__('on ', 'Maxshop').' '.$comment->post_title?>" class="pull-left">
							<?php echo get_avatar($comment, '60')?>
						</a>
						<div class="media-body">
							<a href="<?php echo get_permalink($comment->ID).'#comment-'.$comment->comment_ID?>" title="<?php echo strip_tags($comment->comment_author).' '.__('on', 'Maxshop').' '.$comment->post_title?>">
								<?php echo strip_tags($comment->comment_author)?>
							</a>
							<p><?php echo strip_tags($comment->com_excerpt)?></p>
							<time datetime="<?php echo get_the_time('Y-m-d'); ?>"><i class="icon-calendar"></i><?php echo get_the_time('F d, Y'); ?></time>
						</div>
					</div>
...
/* ***************************************************** 
 * Plugin Name: Maxshop Contact Widget
...
class alc_Contact_Form extends WP_Widget {
	...
	function alc_Contact_Form() {
		$widget_ops = array('classname' => 'alc_contact_form_entries', 'description' => __("Contact widget", 'Maxshop') );
		$this->WP_Widget('alc_Contact_Form', __('Maxshop - Contact Form', 'Maxshop'), $widget_ops);
	}

	function widget($args, $instance) {
		extract($args);
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Contact Us', 'Maxshop') : $instance['title'], $instance);
		$email = apply_filters('widget_title', empty($instance['email']) ? __('', 'Maxshop') : $instance['email'], $instance);
		$success = apply_filters('widget_title', empty($instance['success']) ? __('Thank you, e-mail sent.', 'Maxshop') : $instance['success'], $instance);
        ...
        ?>                
			<form action="#" method="post" id="contactFormWidget">
				<div class="row-fluid">
					<div class="span6">
						<input type="text" name="wname" id="wname" value="" size="22" placeholder="<?php _e('Name', 'Maxshop')?>"/>
					</div>
					<div class="span6">
						<input type="text" name="wemail" id="wemail" value="" size="22" placeholder="<?php _e('Email', 'Maxshop')?>" />
					</div>
				</div>	
				<div class="row-fluid">				
					<div class="span12">
						<textarea name="wmessage" id="wmessage" cols="60" rows="10" placeholder="<?php _e('Message', 'Maxshop')?>"></textarea>
					</div>
				</div>
				<div class="row-fluid">
					<div class="span12">
						<input type="submit" id="wformsend" value="<?php _e('Send', 'Maxshop')?>" class="btn" name="wsend" />
					</div>
				</div>
				...
				<div class="widgetinfo"><i class="icon-envelope"></i><?php echo $success?></div>
			</form>
		<?php
		echo $after_widget;
	}
	...
	?>
		...
		<div>
        	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:<br />', 'Maxshop'); ?>
			<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></label></p>
		</div>
        <div>
        	<label for="<?php echo $this->get_field_id('email'); ?>"><?php _e('Email Address:<br />', 'Maxshop'); ?>
        ...
        <div>
        	<label for="<?php echo $this->get_field_id('success'); ?>"><?php _e('Success Message:<br />', 'Maxshop'); ?>
