...
<div id="order_review" class="clearfix">
	...
	<li class="product-quantity"><?php _e('Quantity', 'woocommerce'); ?></li>
        <li class="product-price"><?php _e('Price', 'woocommerce'); ?></li>
	...
	echo '<ul class = "' . esc_attr( apply_filters('woocommerce_checkout_table_item_class', 'checkout_table_item', $values, $item_id ) ) . '">
		<li class="product-name">'.$_product->get_title().$woocommerce->cart->get_item_data( $values ).'</li>
		<li class="product-quantity">'.$values['quantity'].'</li>
                <li class="product-price">'.$_product->get_price().'</li>
		<li class="product-total">' . apply_filters( 'woocommerce_checkout_item_subtotal', $woocommerce->cart->get_product_subtotal( $_product, $values['quantity'] ), $values, $item_id ) . '</li>
              </ul>';
	endif;
	endforeach;
	endif;
    ...
    <div class="totle">
		...
		// Print a single available shipping method as plain text
			...
			// Show multiple shipping methods
				...
				foreach ( $available_methods as $method )
                                    echo '<option value="' . esc_attr( $method->id ) . '" ' . selected( $method->id, $_SESSION['shipping_method'], false ) . '>' . strip_tags( $method->full_label ) . '</option>';
                                    echo '</select>';
                            } else {
                            echo '<ul id="shipping_method">';
                            foreach ( $available_methods as $method )
                                echo '<li><input type="radio" name="shipping_method" id="shipping_method_' . sanitize_title( $method->id ) . '" value="' . esc_attr( $method->id ) . '" ' . checked( $method->id, $_SESSION['shipping_method'], false) . ' /> <label for="shipping_method_' . sanitize_title( $method->id ) . '">' . $method->full_label . '</label></li>';
                                echo '</ul>';
                            }
                        }
		...
		<td><?php echo $woocommerce->cart->get_total(); ?></td>
            </tr>
...
<h5><small><?php _e('Step ', 'woocommerce'); ?></small><a href="#"><?php _e('Payment', 'woocommerce'); ?></a></h5>
	...
	<div id="payment" class="clearfix">
		...
		<div class="form-row place-order">
			...
			<input type="submit" class="red-button alt" name="woocommerce_checkout_place_order" id="place_order" value="<?php echo apply_filters('woocommerce_order_button_text', __('Place order', 'woocommerce')); ?>" />
